...
class TrackFeaturesFileHandler:
    ...
    def save_track_features(self, file_location : Path, tf : TrackFeatures):
        np.savez(file_location, F=tf.F, T=tf.T, S_mag=tf.S_mag, dt=tf.dt, frame_width=tf.frame_width, frame_height=tf.frame_height, time_resolution=tf.time_resolution)
